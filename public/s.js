...
function t(el, text) { if(text) { el.innerText = text } else  return el.innerText }

function to(el, child, prepend = 0) {
    if(typeof el === 'string') { el = e(el) }
    if(typeof child === 'object') { child = ce(child) }
    prepend ? el.prepend(child) : el.append(child)
    return child
}
